...
```ts
// system = PERSONA_TEXT（唯一段落，无拼接）
let messages = composeMessages(PERSONA_TEXT, history, ctx.userText);

// 工具 = 极简双工具（bash + str_replace_editor）——不拼 toolsCore
tools: [...minimalTools({ cwd: process.env.CORE_DIR })],
```

其余与 standard 同构：`loadHistory(ctx.memory, 8)` 记忆 → LLM↔工具循环 → 写回历史。lib 的 `createHarness` 只是 HTTP 壳 + 资源注入，不藏任何编排；harness 就是这份代码本身。

## 依赖

- `comrade-harness-lib`（git 依赖 + commit id）：节点函数 + 运行时壳。**只 import 需要的节点**，工具包（`toolsCore`）不要带进来。
- `dsh-minimal`（git 依赖 + commit id）：极简模式的 persona 与双工具，模型可见层单一定义源。
- 本地开发：`bun run local:on` 把两个依赖切到本地 link（改 lib/库即时生效），`local:off` 还原 git 依赖；package.json/bun.lock 已 skip-worktree 不会被误提交。
