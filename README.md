...
`standardFlow` 内部是 `loadContext`（默认全量历史进上下文，含 step 过程痕迹——那是模型自己的轨迹，映射为 assistant，不是外来注入）→ `agentLoop`（LLM↔工具循环）→ `saveTurn`（写回历史）。lib 的 `createHarness` 只是 HTTP 壳 + 资源注入，不藏任何编排；三个子图都是普通函数，可逐层深入定制（选项 → hooks → 整层换函数 → 手拼子图）。
