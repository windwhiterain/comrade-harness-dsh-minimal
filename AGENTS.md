...
- 你的代码极简到只有两个工具（bash + str_replace_editor）和一句 persona——**别往模型可见层加 lib 的工具/提示词**（`toolsCore`、`buildSystemPrompt` 会破坏过拟合的极简行为；你的数据流来自 lib 的 `standardFlow({ systemPrompt: PERSONA_TEXT })` 子图，那是循环代码不是 prompt，没问题）。
